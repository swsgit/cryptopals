...
int main(void) {
  ...
  for (size_t i = 0; i < 255; i++, key++) {
    dec = single_xor((unsigned char *)str, key, str_len);
    score = strscore((unsigned char *)dec, keymap, keymap_len, str_len);
    certain = (score + 1 >= str_len) ? 1 : 0;
    if (certain) {
      print_hex(dec, str_len);
    }
    free(dec);
  }
